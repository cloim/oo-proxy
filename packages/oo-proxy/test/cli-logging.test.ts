...
describe("cli logging", () => {
	...
	test("formats the startup banner with spacing", () => {
		expect(
			toStartupMessage("http://127.0.0.1:40531/v1", [
				"gpt-5.4",
				"gpt-5.3-codex",
			]),
		).toBe(
	...
	test("relabels AI SDK warnings for the CLI", () => {
		...
		expect(info).toHaveBeenNthCalledWith(
			2,
			"oo-proxy Warning System: To turn off warning logging, set the AI_SDK_LOG_WARNINGS global to false.",
		)
		expect(warn).toHaveBeenCalledWith(
			'oo-proxy Warning (openai.responses / gpt-5.3-codex): The feature "temperature" is not supported. temperature is not supported for reasoning models',
		)
	})
})
