...
import { createOoProxyFetchHandler } from "../src/index.js"

const createAuthFile = async (): Promise<string> => {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "oo-proxy-server-"))
...
describe("openai oauth server", () => {
	...
	test("lists configured models", async () => {
		const handler = createOoProxyFetchHandler({
