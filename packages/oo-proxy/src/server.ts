...
import {
	createOoProxy,
	type OoProxyProvider,
} from "../../oo-proxy-provider/src/index.js"
...
import type {
	...
	RunningOoProxyServer,
} from "./types.js"

const handleRoutes = async (
	request: Request,
	settings: OoProxyServerOptions,
	provider: OoProxyProvider,
...
export const createOoProxyFetchHandler = (
	settings: OoProxyServerOptions = {},
	...
	const provider = createOoProxy(sharedSettings)
...
export const startOoProxyServer = async (
	settings: OoProxyServerOptions = {},
): Promise<RunningOoProxyServer> => {
	const host = settings.host ?? DEFAULT_HOST
	const port = settings.port ?? DEFAULT_PORT
	const handler = createOoProxyFetchHandler(settings)
